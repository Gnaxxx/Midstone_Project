...
void Scene4::HandleEvents(const SDL_Event& sdlEvent) {
	...
	if (sdlEvent.type == SDL_EventType::SDL_KEYDOWN) {
		if (sdlEvent.key.keysym.scancode == SDL_SCANCODE_D) {
			for (int i = 0; i < balls.size(); ++i) {
				balls[i]->ApplyForce(Vec3(r.rand(10.0f, 30.0f), 0.0f, 0.0f));
		...
		if (sdlEvent.key.keysym.scancode == SDL_SCANCODE_D) {
			for (int i = 0; i < balls.size(); ++i) {
				balls[i]->ApplyForce(Vec3(0.0f, -9.8f, 0.0f));
			}
		}
	}
...
void Scene4::Render() {
	...
	for (int i = 0; i < balls.size(); ++i) {
		...
		square.w = w;
		square.h = h;
		SDL_RenderCopyEx(renderer, balls[i]->getTexture(), nullptr, &square, balls[i]->getRotationZ(), nullptr, SDL_FLIP_NONE);
